    ...
    var body: some View {
        ...
        .highPriorityGesture(
            DragGesture(minimumDistance: 0, coordinateSpace: .named("graph-space"))
                .onChanged { value in
                    if model.graphState.dragStartPort != handle {
                        model.graphState.beginEdgeDrag(from: handle, at: portCenter(for: handle) ?? value.startLocation)
                    }

                    let hovered = nearestInputHandle(to: value.location)
                    model.graphState.updateEdgeDrag(point: value.location, hoveredInput: hovered)
                }
                .onEnded { value in
                    let hovered = nearestInputHandle(to: value.location)
                    model.graphState.endEdgeDrag(over: hovered)
                }
        )
        .background(
            GeometryReader { proxy in
